...
int aviBufferFreeSize(AviBuffer *aBuffer) {
	if ((unsigned int)aBuffer->writePointer == (unsigned int)aBuffer->readPointer && aBuffer->amountLeft == 0) {
		return INPUT_BUFFER_CHUNK_NUMBER*INPUT_BUFFER_CHUNK_SIZE;
	}

	if ((unsigned int)aBuffer->writePointer > (unsigned int)aBuffer->readPointer) {
		return INPUT_BUFFER_CHUNK_NUMBER*INPUT_BUFFER_CHUNK_SIZE - (unsigned int)aBuffer->writePointer + (unsigned int)aBuffer->readPointer;
	} else {
		if ((unsigned int)aBuffer->writePointer <= (unsigned int)aBuffer->readPointer) {
			return (unsigned int)aBuffer->readPointer - (unsigned int)aBuffer->writePointer;
		}
	}
}
...
int aviBufferLoadChunk (AviBuffer* aBuffer) {
	...
	len = fread(aBuffer->writePointer, 1, INPUT_BUFFER_CHUNK_SIZE, aBuffer->aviFile);
	...
	if (aBuffer->writePointer > aBuffer->bufferStart + (INPUT_BUFFER_CHUNK_NUMBER-1)*INPUT_BUFFER_CHUNK_SIZE) {
		printf("Wrapping buffer position\n");
		// We copy the last chunk at the first chunk position
		memcpy(aBuffer->bufferStart, aBuffer->bufferStart + (INPUT_BUFFER_CHUNK_NUMBER-1)*INPUT_BUFFER_CHUNK_SIZE, len);
		// We set the pointer to the second chunk position
		aBuffer->writePointer = aBuffer->bufferStart + INPUT_BUFFER_CHUNK_SIZE;
	}
...
void getStreamNextDataChunk(AviBuffer* aBuffer) {
	do {
		...
		//aviBuffer.moviRemain -= aviBuffer.nextChunkOffset; // TO CHECK!!!!
                ...
                if (aBuffer->readPointer >= aBuffer->bufferStart + (INPUT_BUFFER_CHUNK_NUMBER-1)*INPUT_BUFFER_CHUNK_SIZE) {
                	aBuffer->readPointer = aBuffer->readPointer - (INPUT_BUFFER_CHUNK_NUMBER-1)*INPUT_BUFFER_CHUNK_SIZE;
                }

                // Ensure there is enough data in the buffer
                while (aBuffer->amountLeft < AVI_MIN_CHUNKS * INPUT_BUFFER_CHUNK_SIZE && aviBufferLoadChunk(aBuffer) > 0);

                memcpy(&(aBuffer->chunkHeader), aBuffer->readPointer, sizeof(AviChunkHeader));
	
                if (aBuffer->chunkHeader.fourcc == LIST_ID) { // List
			...
			if (aBuffer->chunkHeader.fourcc == aBuffer->videoFourcc) {
				...
				if (aBuffer->chunkHeader.fourcc ==  aBuffer->audioFourcc) {
					...
					aBuffer->chunkData.dataType = AVI_DATATYPE_IGNORE;
                        ...
                        aBuffer->chunkData.currentPosition = aBuffer->readPointer + sizeof(AviChunkHeader);
                        aBuffer->chunkData.amountLeft = aBuffer->chunkHeader.size;
                        aBuffer->nextChunkOffset = aBuffer->chunkHeader.size + sizeof(AviChunkHeader);
                }
	} while (
        	((aBuffer->chunkHeader.fourcc != aBuffer->videoFourcc && aBuffer->chunkHeader.fourcc != aBuffer->audioFourcc)
                || aBuffer->chunkData.amountLeft == 0)    /* skip chunks that don't contain any data */
...
void aviUsedAmount(AviBuffer* aBuffer, int amount) {
	...
	if (aBuffer->chunkData.amountLeft <= 0 || amount == 0) {
		...
		if (aBuffer->chunkHeader.fourcc != aBuffer->videoFourcc && aBuffer->chunkHeader.fourcc != aBuffer->audioFourcc) {
...
bool aviBufferInit (FILE* aviFile, int *frameWidth, int *frameHeight, int *frameRate, int *numFrames) {
	...
	if (feof(aviBuffer.aviFile)) {
		...
		fread (&aviIndexEntry, sizeof(AviIndexEntry), 1, aviBuffer.aviFile);
		aviBuffer.indexOffset = aviMoviListStart + sizeof (AviListHeader) - aviIndexEntry.offset;
	}

	// Allocate memory for buffer
	aviBuffer.bufferStart = (u8*) malloc(INPUT_BUFFER_CHUNK_NUMBER*INPUT_BUFFER_CHUNK_SIZE);
	...
	fseek(aviBuffer.aviFile, aviMoviListStart + sizeof (AviListHeader), SEEK_SET);
