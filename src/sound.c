...
audio_linear_dither (unsigned int bits, mad_fixed_t sample,
		     struct audio_dither *dither, struct audio_stats *stats)
{
...
void
mad_decode (unsigned char *istream,
	    signed short *ostream, int istream_size, DecodeResult * result)
{
  int i;
  int remaining;
  ...
  memcpy (mad_audio_buffer + mad_audio_buffer_used_size, istream,
	  istream_size);
  ...
  switch (Synth.pcm.channels)
	...
	{
	  ostream[numOut++] =
	    audio_linear_dither (16, (*left_ch++), &Audio_dither,
				 &Audio_stats);
	  ostream[numOut++] =
	    audio_linear_dither (16, (*right_ch++), &Audio_dither,
				 &Audio_stats);
	}
      break;
    case 1:
