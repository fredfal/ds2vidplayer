...
typedef struct
{
  ...
  void *(*fopen_function) (const char *archive_filename, const char *filename,
			   const char *mode);
  void (*fclose_function) (void *stream);
    size_t (*fread_function) (void *ptr, size_t size, size_t nmemb,
			      void *stream);
...
typedef struct
{
  ...
  image_data_s *(*load_image) (void *fp, int max_width, int max_height,
			       size_t (*fread_fonction) (void *ptr,
							 size_t size,
							 size_t nmemb,
							 void *stream),
			       int (*fseek_function) (void *stream,
						      long offset,
						      int whence),
