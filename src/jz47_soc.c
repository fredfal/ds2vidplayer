...
static volatile unsigned char *ipu_vbase = NULL, *img_ptr =
  NULL, *frame_buffer = NULL;
...
get_fbaddr_info ()
{
  ...
  if ((frame_buffer =
       (uint8_t *) mmap (0, fbmemlen, PROT_READ | PROT_WRITE, MAP_SHARED,
			 fbfd, 0)) == (uint8_t *) - 1)
    {
  ...
  printf
    ("++ jz47 fb_w=%d, fb_h=%d, phy_fb=0x%x, fbmemlen=%d, fb_line_len=%d\n",
     fb_w, fb_h, phy_fb, fbmemlen, fb_line_len);
}

unsigned int
get_phy_addr (unsigned int vaddr)
{
  int i;
...
void *
jz4740_alloc_frame (int align, int size)
{
  ...
  if (memdev_count < MEM_ALLOC_DEV_NUM)
      ...
      else
	{
	  ...
	  memdev_count++;
	  printf
	    ("jz Dev alloc 2: vaddr = 0x%x, paddr = 0x%x size = 0x%x ++\n",
...
/* set ipu data format.  */
int
jz47_set_ipu_csc_cfg (SwsContext * c, int outW, int outH, int Wdiff,
		      int Hdiff)
{
...
// Set GS register
  REG32 (ipu_vbase + REG_IN_FM_GS) =
    IN_FM_W ((c->srcW - Wdiff) & ~0x1) | IN_FM_H ((c->srcH - Hdiff) & ~0x1);
...
int
find_ipu_ratio_factor (float ratio)
{
  int i, sel;
  float diff, min = 100;
  sel = ipu_rtable_len;
...
int resize_out_cal (int insize, int outsize, int srcN, int dstM, int upScale,
		    int *diff);
int
jz47_set_ipu_resize (SwsContext * c, int *outWp, int *outHp,
		     int *Wdiff, int *Hdiff)
{
...
// Don't need do resize ?
  if (((c->srcW == rsize_w) && (c->srcH == rsize_h))
      || (rsize_w > 2 * c->srcW) || (rsize_h > 2 * c->srcH))
    {
...
int
resize_out_cal (int insize, int outsize, int srcN, int dstM,
...
int
resize_lut_cal (int srcN, int dstM, int upScale, rsz_lut lut[])
{
  int i, t, x;
...
int
jz47_ipu_init (struct vf_instance_s *vf)
{
  ...
  if (!toggle_flag && memdev[0].vaddr && memdev[0].paddr)
    {
      ...
      sprintf (cmdbuf, "echo 00%08x 00%08x 00%08x 00%08x > %s", cur_pid,
	       memdev[0].vaddr, memdev[0].paddr, 0x400000, "/proc/jz/ipu");
...
void
ipu_outsize_changed (int x, int y, int w, int h)
{
  //int offset_x =x;
  //int offset_y =y;
  if ((w == 0) && (h == 0))
...
int
jz47_put_image_with_ipu (struct vf_instance_s *vf, mp_image_t * mpi,
			 double pts)
{
  unsigned int paddr;
  //SwsContext *c = vf->priv->ctx;
...
// set Y(mpi->planes[0]), U(mpi->planes[1]), V addr (mpi->planes[2])
  ...
  if (paddr == 0)
    {
      printf
	("++ Can not found Y buffer(0x%x) physical addr since addr errors +++\n",
...
// set U addr register (mpi->planes[1])
  ...
  if (paddr == 0)
    {
      printf
	("++ Can not found U buffer(0x%x) physical addr since addr errors +++\n",
...
// set V addr register (mpi->planes[2])
  ...
  if (paddr == 0)
    {
      printf
	("++ Can not found V buffer(0x%x) physical addr since addr errors +++\n",
...
static void
scale (struct SwsContext *sws1, struct SwsContext *sws2, uint8_t * src[3],
       int src_stride[3], int y, int h, uint8_t * dst[3], int dst_stride[3],
       int interlaced)
{
  if (interlaced)
    {
...
int
jz47_put_image (struct vf_instance_s *vf, mp_image_t * mpi, double pts)
{
  ...
  if (!(mpi->flags & MP_IMGFLAG_DRAW_CALLBACK && dmpi))
    {
      ...
      dmpi = vf_get_image (vf->next, vf->priv->fmt,
			   ...
			   MP_IMGFLAG_PREFER_ALIGNED_STRIDE, vf->priv->w,
			   vf->priv->h);
#endif
      if (mpi->pict_type == 1)
	drop_visual = 0;
      if ((drop_visual == 0) && (!jz47_put_image_with_ipu (vf, mpi, pts)))
	scale (vf->priv->ctx, vf->priv->ctx, mpi->planes, mpi->stride, 0,
	       mpi->h, dmpi->planes, dmpi->stride, vf->priv->interlaced);
    }
#if 0
