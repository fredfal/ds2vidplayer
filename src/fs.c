...
const char *fs_get_fileextension(const char *filename)
{
        ...
        while (position > filename && *position != CONFIG_FILEEXTENSION_SEPARATOR && *position != CONFIG_DIR_SEPARATOR)
                position--;
...
const char *fs_basename(const char *pathname)
{
...
t_fs_filetype_e fs_get_filetype(const char *filename)
{
        ...
        t_fs_filetype_entry *filetype_entry;ft_table;
        ...
        {
                if (strcasecmp(fs_basename(filename), specfiletype_entry->filename) == 0)
                {
