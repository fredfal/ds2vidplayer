...
int frameconversion_jz4740_ipu(unsigned char* source_address, unsigned char* destination_address, int source_width, int source_height) {
	...
	printf("STOP %d - %d - %d - %d\n", polling_end_flag(IPU_P_BASE), ipu_is_enabled(IPU_P_BASE), __dmac_test_halt_error(), __dmac_test_addr_error());
	...
	stop_ipu(IPU_P_BASE);
        ...
        REG32(IPU_P_BASE + REG_IN_FM_GS) = IN_FM_W(source_width) | IN_FM_H(source_height);
        REG32(IPU_P_BASE + REG_Y_STRIDE) = source_width;
        REG32(IPU_P_BASE + REG_UV_STRIDE) = U_STRIDE(source_width) | V_STRIDE(source_width);

        // Set source and destination addresses
	int source_size = source_width * source_height;
        REG32(IPU_P_BASE + REG_Y_ADDR) = PHYSADDR((unsigned int)source_address);
        REG32(IPU_P_BASE + REG_U_ADDR) = PHYSADDR((unsigned int)source_address + source_size);
        REG32(IPU_P_BASE + REG_V_ADDR) = PHYSADDR((unsigned int)source_address + source_size + source_size/4);
        REG32(IPU_P_BASE + REG_OUT_ADDR) = PHYSADDR((unsigned int)destination_address);

        // Set destination size and stride
        REG32(IPU_P_BASE + REG_OUT_GS) = OUT_FM_W(SCREEN_WIDTH << 1) | OUT_FM_H(SCREEN_HEIGHT << 1);
	...
	run_ipu(IPU_P_BASE);
	printf("RUN: %d - %d\n", polling_end_flag(IPU_P_BASE), ipu_is_enabled(IPU_P_BASE));
	printf("w:%d - h:%d\n", source_width, source_height);
	printf("IY:%x IU:%x IV:%x\n", REG32(IPU_P_BASE + REG_Y_ADDR), REG32(IPU_P_BASE + REG_U_ADDR), REG32(IPU_P_BASE + REG_V_ADDR));
	printf("%x %x %x\n", source_address, (unsigned int)source_address + source_width, (unsigned int)(source_address + source_width));
