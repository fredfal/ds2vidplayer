...
------------------------------------------------------------------------------*/
static u32
bitmap_code (unsigned char *code, unsigned char *bitmap)
{
  unsigned char *map;
  u8 a, b;
  u32 len;
...
------------------------------------------------------------------------------*/
static int
parse_bdf (char *filename, u32 start, u32 span,
	   struct bdflibinfo *bdflibinfop, u32 method)
{
  FILE *fp;
  char string[256];
  char map[256];
  ...
  bitbuff =
    (unsigned char *)
    malloc ((bdflibinfop->width * bdflibinfop->height * span) >> 3);
  if (bitbuff == NULL)
    {
      ret = -5;
      goto parse_bdf_error;
  ...
  while (1)
      ...
      while (1)
	{
	  ...
	  if (pt == NULL)
	    goto parse_bdf_error;
	  if (!(strncasecmp (string, "ENCODING ", 9)))
	    break;
	}
...
parse_bdf_error:
  ...
  if (ret < 0)
  ...
  else
    {
      ...
      bdflibinfop->mapmem =
	(unsigned char *) realloc ((void *) bdflibinfop->mapmem, length);
    }
...
------------------------------------------------------------------------------*/
int
dump2odf (char *filename, struct bdflibinfo *bdflibinfop)
{
  char *pt;
  char string[256];
  FILE *fp;
...
------------------------------------------------------------------------------*/
int
init_from_odf (char *filename, struct bdflibinfo *bdflibinfop)
{
  FILE *fp;
  char buff[512];
  char *pt;
...
BDF_font_init (void)
{
  ...
  err = parse_bdf (tmp_path, 32 /* from SPACE */ ,
		   8564
		   /* to one past the last character, "DOWNWARDS ARROW" */ ,
		   &bdflib_info[0], 1);
  if (err < 0)
    {
...
------------------------------------------------------------------------------*/
u32
BDF_render16_ucs (void *screen_address, u32 screen_w, u32 v_align, u32 back,
		  u32 front, u16 ch)
{
  ...
  if (v_align == 0)		//v align bottom
    screen =
      (unsigned short *) screen_address + x_off + (fonts_max_height - height -
						   y_off) * screen_w;
...
------------------------------------------------------------------------------*/
static u32
BDF_render16_font (char *screen_address, u32 back, u32 front, u16 ch)
{
  unsigned short *screen, *screenp;
  unsigned char *map;
  ...
  screen =
    (unsigned short *) screen_address + x_off + (font_height - height -
						 y_off) * SCREEN_WIDTH;
...
------------------------------------------------------------------------------*/
void
BDF_render_string (void *screen_address, u32 x, u32 y, u32 back, u32 front,
		   char *string)
{
  char *pt;
...
unsigned char *
skip_utf8_unit (unsigned char *utf8, unsigned int num)
{
  while (num--)
    {
      unsigned char c = *utf8++;
...
------------------------------------------------------------------------------*/
void
BDF_render_mix (void *screen_address, u32 screen_w, u32 x, u32 y, u32 v_align,
		u32 back, u32 front, char *string)
{
  ...
  while (*pt)
      ...
      if ((screenp + cmp) >= line_start)
	{
      ...
      width =
	BDF_render16_ucs ((unsigned short *) screenp, screen_w, v_align, back,
			  front, unicode);
      screenp += width * 2;
    }
}
...
------------------------------------------------------------------------------*/
u32
BDF_cut_string (char *string, u32 width, u32 direction)
{
  char *pt;
  u16 unicode[256];
  u32 len, xw;
...
------------------------------------------------------------------------------*/
u32
BDF_cut_unicode (u16 * unicodes, u32 len, u32 width, u32 direction)
{
